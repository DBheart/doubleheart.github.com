...
## 번외 1. [GitHub로 프로젝트 관리하기](github_issueManager.md)
## 번외 2. [Git Page 사용방법](./github/github_page.md)
## 번외 3. [git 레퍼런스](https://git-scm.com/book/ko/v2)

GitHub, Git Flow을 이용하고 있는 현재 프로젝트를 기반으로 Git을 어떻게 쓰면 좋일지 정리하는 글이다. Git Lab나 그외 개인서버에서도 사용할수 있을거라고 생각된다.

번외로 GitHub Page에 대해서 쓰고 있다. GitHub Page는 내가 이제부터 공부하고 정리할 내용을 담는 곳이어서 미리 정리하여 놓았다.

git을 잘쓰기 위해서는 위해서는 3가지의 status와 branch전략을 종합적으로 이해하여야 한다. 권장하는 branch전략이 git flow이고 우아한 형제들이 그것을 fork포함해서 어떻게 쓰고 있는지 소개하고 있어서 우아한형제들에 대한 글이 많이 있다.

Git의 동작원리를 이해하기 위해서는 아래 글을 읽고 오는게 좋다.
* 레퍼런스 
...
1장에서는 로컬에서 개발하고 remote서버에 반영하는 방법에 대해서 언급하고, 2장에서는 flow을 어떻게 사용하는지에 대해서 우아한형제들것을 보면서 정리하였다.
